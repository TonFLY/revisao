...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (const [domain] of DOMAIN_WEIGHTS) {
      ...
      const r = await pool.request()
        ...
        .query(`
          ...
          SELECT TOP (@n) q.id, q.case_id, q.domain_code, q.origin
    ...
    // Fallback: se o banco V2/REAL não tiver questões suficientes,
    // complete o simulado com questões existentes (origin=legacy).
    // Decoreba fica fora do Simulado REAL.
    if (selected.length < requestedCount) {
      const missing = requestedCount - selected.length;
      const exclude = selected.map(x => x.id);
      const req = pool.request()
      ...
      if (exclude.length) {
        const ps = exclude.map((id, i) => {
          req.input(`legacy_x${i}`, sql.Int, id);
          return `@legacy_x${i}`;
        });
        whereExclude = `AND q.id NOT IN (${ps.join(',')})`;
      }
      ...
      const r = await req.query(`
        ...
        WHERE q.user_id=@uid
          AND q.exam=@exam
          AND q.origin<>N'real'
          AND ISNULL(q.topic,N'') NOT LIKE N'Decoreba%'
          AND (q.correct IS NOT NULL OR q.correct_json IS NOT NULL)
          ${whereExclude}
        ORDER BY CASE WHEN p.last_attempt IS NULL THEN 0 ELSE 1 END,
                 p.last_attempt ASC,
                 NEWID();
    ...
    if (!selected.length) {
      return res.status(409).json({
        error: 'Nenhuma questão disponível para este usuário/prova.'
      });
    }
    ...
    return res.status(201).json({
      ...
      blueprint: q,
      source_mix: {
        real: selected.filter(x => x.origin === 'real').length,
        existing: selected.filter(x => x.origin !== 'real').length,
      },
    });
  } catch (e) {
